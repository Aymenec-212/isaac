...
| L-42 | **The contrast script found five shortfalls F1 measured but did not have the mandate to change — one of them an invisible control** | Recorded the day the measuring started (2026-09-14), because a number nobody wrote down is a number nobody fixes. **The one to read first: "Changer de jeton" is invisible at rest, 1.00:1.** `.btn-quiet` sets `color: var(--ink)` and the rail it sits in is `background: var(--ink)`, so the only authentication affordance in the app is ink on ink until `:hover` paints `--surface` behind it; its `--grout` border is all that shows. Computed from the cascade, not observed in a browser. The other four: the `--grout` hairline that is a text input's only boundary measures 1.39:1 against `--surface` where WCAG 1.4.11 asks 3:1; `.seg-edit` at `opacity: 0.45` measures 2.06:1 at rest, reaching full `--azure` only on hover and focus; `:focus-visible`'s `--azure` ring on the dark rail measures 2.37:1; and `.evidence-link` at `opacity: 0.8` passes at 8.63:1 but drops to 0.4 when disabled. **Not fixed in F1 because every fix is a visual decision**, and F1's mandate was re-anchoring tokens with the appearance held still — the enhancement plan parks the rail button as its own open question (§6.2, magic links / L-11). Each is pinned at today's ratio in `check-contrast.mjs` and fails the build if it gets worse | F2 for the three that are palette decisions; the rail button is Aymen's call now that it has a number — it is a two-line fix and it is the only one a person cannot work around |
...
**What F1 deliberately did not do is change how anything looks.** That is F2's
mandate, and it is also why the five contrast gaps the new script found are
recorded (L-42) rather than fixed — including the one worth acting on before
F2: **"Changer de jeton" renders ink-on-ink in the dark rail, 1.00:1.**
