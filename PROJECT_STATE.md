...
| Next action | **Slice 6A**, in this order: `/readyz` + `/health/deps` per dependency, then transcript search (FR-10), then a long single-user run against the real MLX runtime, then the full-lifecycle test. Health first because it is what tells you *which* dependency broke a local run, and Phase A is about running meetings repeatedly |
