...
| L-42 | **The contrast script found five shortfalls the first time anything measured this palette. One was a bug and is fixed; four remain** | Recorded 2026-09-14, because a number nobody wrote down is a number nobody fixes. **The bug, now closed: "Changer de jeton" was invisible at rest.** `.btn-quiet` sets `color: var(--ink)` and the rail it sits in is `background: var(--ink)`, so the only authentication affordance in the app was ink on ink — its `--grout` border the only thing showing — until `:hover` painted `--surface` behind it. Found by computation, then **observed in Chromium**: before the fix the button's computed `color` and the rail's computed `background-color` are both `rgb(17, 33, 45)`, exactly 1.00:1. Fixed in F1 on Aymen's call, by two rules that give a quiet button the colour of the surface it sits on: 14.33:1 at rest, 16.42:1 on hover, both now asserted pairs rather than gaps. **The four that remain are visual decisions, not defects**, which is why F1 measured and pinned them instead: the `--grout` hairline that is a text input's only boundary measures 1.39:1 against `--surface` where WCAG 1.4.11 asks 3:1; `.seg-edit` at `opacity: 0.45` measures 2.06:1 at rest, reaching full `--azure` only on hover and focus; `:focus-visible`'s `--azure` ring on the dark rail measures 2.37:1, so a keyboard user can now see that button but still not see that it is focused; and `.evidence-link` at `opacity: 0.8` passes at 8.63:1 but drops to 0.4 when disabled. Each is pinned at today's ratio in `check-contrast.mjs` and fails the build if it gets worse | F2, which owns the appearance. The rail focus ring is one line (`.rail :focus-visible { outline-color: var(--porcelain) }`, 14.33:1) whenever the rail's focus treatment is decided |
...
**F1 is implemented and gated for review.** It re-anchored the visual system —
...
**Nobody has seen the F-series rendered as a design.** There is no visual
regression baseline in this repository and no CI to hold one (L-18), so every
gate in the plan is run by hand, and the plan's own appendix says the same:
the measurements are facts, the design is a proposal. F2 introduces the ground
and glass; read §2 of the plan, and its rule that glass never touches a
text-reading surface, before starting it.
