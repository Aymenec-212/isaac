...
attributed transcription and both review persisted transcript/intelligence after
ending. The maintainer selected direct WebRTC with TURN fallback, Azure using
available credit, and continued voice with a warning during ASR outages.
...
to finish all single-user work before GPU validation. Required multi-user metrics
move into R6; unrelated single-user work remains deferred. Slice 6R stays complete;
Slice 6A's remaining metrics are not marked complete. Four participants remain a
later gate. MLX remains a one-stream native development option; model types remain
behind `StreamingRecognizer` / `KyutaiBackend`.

**Evidence, not a new production claim:** on 2026-09-11, backend unit tests were
`305 passed, 94 deselected`, frontend unit tests `91 passed`, and frontend typecheck
passed. No GPU, cross-network or browser-call gate was run. Azure CLI reported an
enabled subscription named Microsoft Azure Sponsorship despite the stated free
...
**R3 evidence (2026-09-13):** full backend suite **444 passed, 1 deselected**
with explicit fake ASR/LLM and isolated native PostgreSQL 14; includes 14 new
lifecycle unit tests and 8 new gateway/database tests. Frontend **93 passed**,
typecheck/build pass. The accelerated-hour test passes (**1 passed, 444 deselected**,
68.63 seconds); 26 focused lifecycle/persistence tests were rerun after final review.
Backend ruff/format pass and OpenAPI has no drift. Mypy
retains eight existing MLX errors, with no new errors (100 source files).
Real GPU interruption, PostgreSQL 16 container parity and browser-call acceptance
remain unverified. Docker Desktop remains manually paused.

**R4 evidence (2026-09-13):** Docker is working. Isolated PostgreSQL 16:
450 backend tests pass (one slow deselected), accelerated hour passes separately
in 67.02 seconds; six identity integration tests also pass after the roster fix.
Frontend: 99 tests, typecheck/build and all 20 browser tests pass with fake ASR/LLM.
...
| Field | Value |
|---|---|
| Repository | **`main` through PR #16 (2026-09-10), which merged Slice 6R item 7.** Work branch `claude/sharp-dijkstra-uf34s8` is synced to that same commit (`29c7b3d`) and carries nothing unmerged; it is restarted from `main` after each merge |
...
| Next action | **Review R4** and `docs/r4-identity-review.md`; R5 follows merge. Resolve quota/eligibility and review region parameters before real GPU testing. |
| Next gate | **R4 identity review and real-GPU acceptance, still open.** The overall two-user gate is a real 30-minute cross-network WebRTC call with two concurrent Rust ASR streams and persisted, reviewable Slice 5 outputs. **Not implemented or verified.** Prior Slice 6R evidence remains unchanged. |

---

## 2. Locked decisions

**2026-09-12 amendment:** D-01/ADR-01's companion-only scope is superseded for the
next milestone by the maintainer's explicit choice of in-product voice using
direct WebRTC/TURN. Their PCM transcription ingress decision remains. The old
rows below are historical; [R0](docs/two-user-cloud-architecture.md) records the
current amendment and preserves all unrelated boundaries.
...
| Q9 | Transcript corrections in scope | product | ~~Slice 6R~~ **BUILT 2026-09-10** | ~~out of scope~~ → **in scope, additive only** | **ANSWERED 2026-09-09 by Aymen, and implemented 2026-09-10.** Migration `0002` adds four nullable columns; `text` and `words` are never written again. The second half of the answer came on 2026-09-10 with the item-7 spec: correcting **marks the summary for review and offers a button**, it does not re-run the LLM. Five corrections cost one call, not five — `test_correcting_does_not_re_run_the_summary_by_itself` asserts the absence. Evidence: `test_segment_corrections.py` (16), `e2e/corrections.spec.ts` (5). **ANSWERED 2026-09-09 by Aymen**, who put "manual correction or annotation of transcript segments" in the Slice 6R priority list. This reverses a standing default that had held since 2026-09-03, so it is recorded as a decision rather than absorbed. **The shape is constrained by the slice's design principle:** the raw ASR text and its word timings are never overwritten — a correction is stored alongside, with authorship and a timestamp, and the review view shows the corrected text by default while still being able to reveal what the model said. Destructive editing would make L-28, the 1.43% WER and every `[measure]` row unfalsifiable after the fact. **Still open inside the slice:** whether correcting a cited segment should invalidate or re-run the meeting intelligence that quotes it — a paid LLM call and a product call, deliberately not automatic |
| Q7 | Stack confirmation | product | all | as specified | OPEN |
| Q1 | Companion vs carrier | product | next milestone | direct WebRTC + TURN voice; PCM WS for ASR | **ANSWERED 2026-09-11 → carrier**, superseding the 2026-09-03 companion choice. SPECIFIED in R0, not implemented. |
...
| FR-10 | Transcript review and search | read: Slice 1, search: Slice 6A, **readable presentation: 6R** | **VERIFIED** ✅ fake **+ ✅ M1 + MLX** (presentation only, 2026-09-10) | `GET /transcript?q=` — `test_transcript_search.py` (18), 4 route tests, `highlight.test.ts` (12); presentation by `paragraphs.test.ts` (12) and `e2e/review.spec.ts` (6). Accent-insensitive, a deliberate deviation from §6's "ILIKE for now" because French. Meeting-scoped only — L-33 | Rev 22: the transcript is grouped into speaker-turn paragraphs; highlighting is held by a browser regression test because grouping is what would break it. **Rev 24: Aymen read a real MLX transcript through the grouped view on 2026-09-10 and it held — readable, speaker turns preserved.** Search and citation *offsets* on a real transcript are still only ✅ fake: nobody has typed a query or clicked a citation against real French (L-32 closed the click for the pre-6R page, not this one) |
...
**R0 additions (all unvalidated unless stated):** Azure subscription/credit and
GPU quota eligibility; T4 F16 two-stream compatibility/capacity; direct WebRTC/TURN
cross-network voice; stable reload identity and guest review; real-server tail
drain; four-user mesh capacity. Each has an owner slice and gate in
`docs/two-user-cloud-architecture.md` §§5–9. Empty Azure quota lists are not zero
quota evidence. Historical A-3/A-4/A-8/A-9/A-16 remain open on the real GPU path;
H100 vendor throughput is not evidence for the proposed T4.
...
| `tests/integration/test_segment_corrections.py` | **Q9's contract, held against the database.** A correction never overwrites `text` — asserted on the row, not through the route; the transcript shows the corrected text and still carries the original; a speaker can be reattributed without touching the words and only to someone on the roster; guests, other meetings and other tenants are refused; **search matches the corrected text**, with offsets that slice what the reader sees; correcting bumps `transcript_version`; outputs report which version they came from; correcting three segments enqueues **no** intelligence work; regenerating produces v2 outputs without deleting v1, and pressing the button twice enqueues one job | **16 passing** ✅ fake — re-run on Aymen's M1 2026-09-10 inside the backend's `398 passed, 1 deselected`, and migration `0002` applied cleanly against his database. Running a fake-recognizer test on Apple silicon does not make it a real-model test: this row stays ✅ fake, and the real-runtime correction pass is L-41 |
| `e2e/corrections.spec.ts` | **the correction loop as a person walks it**: edit a segment and see the new text with the original still reachable; the summary declaring itself out of date and one button bringing it back to v2; three corrections *not* re-summarizing; a correction surviving a reload; and cancelling changing nothing | **5 passing** against a real backend, ✅ fake; re-run on the M1 2026-09-10 as part of the 19-spec suite. Caught a real bug in the regenerate polling — see rev 23 |
| `frontend src/review/paragraphs.test.ts` | Slice 6R grouping: speaker change always splits, a long silence splits, the gap is measured from the previous segment's **end**, segments stay separate inside a paragraph, L-28's orphans re-join visually without being altered, and a synthetic 930-segment hour collapses to under 100 blocks with every segment still present exactly once | **12 passing** (vitest) |
| `e2e/review.spec.ts` | **Slice 6R in a browser.** Interim text marked by a word not only a colour; the transcript rendered as speaker-grouped paragraphs with one speaker label and an mm:ss timestamp per turn; a citation still scrolling to its own segment; a search still highlighting inside a grouped paragraph; and failed vs thin summary states each rendering distinctly | **6 passing** against a real backend. **Checked against a concatenating rewrite: 3 of the 6 fail**, and they are exactly the three about transcript rendering — the other three are about behaviour that mutation does not touch |
...
| `frontend/scripts/check-contrast.mjs` | **the palette, measured rather than asserted by eye.** Parses the real `styles.css` — tokens, declarations, `var()` indirection — composites every translucent wash over the surface it actually sits on, folds element `opacity` in as alpha, and checks 43 foreground/background pairs at WCAG 1.4.3 / 1.4.11 thresholds. **F2 taught it the ground**: it composites the three ground tokens to derive the darkest point anything can sit on — rgb(214, 223, 226), a true upper bound because the two radial fields are anchored to opposite corners and never peak together — and measures every glazed surface against that rather than an average, because a translucent surface has a contrast range and not a contrast value. Three known shortfalls are listed separately, each pinned to its measured ratio so they can be paid off but not eroded (L-42) | **43 pairs passing**, wired into `npm test`. **Checked against three deliberate breaks, not assumed load-bearing:** setting `--warn` to the deleted dark-mode `#f0c26b` fails two pairs at 1.47:1 (this is the bug F1 removed); putting a heavy wash back into `--cite-wash-strong` fails the two cited-text pairs; renaming `--danger` fails the run with a named error rather than passing silently |
| `e2e/glass.spec.ts` | **F2's one non-negotiable, held in a browser: glass never touches a surface someone reads.** Asserts the computed `backgroundColor` of `.transcript`, `.outputs` and `.ledger` is fully opaque and their `backdropFilter` is `none`, on the live page *and* after finalization; that `.transcript .para` and `.transcript .seg` introduce no glass of their own; and — through CDP, since Playwright's `emulateMedia` does not carry `prefers-reduced-transparency` — that a reader who asks for less transparency gets opaque chrome and no `backdrop-filter` at all | **3 passing** against a real backend. The second test is a **positive control and the reason the first one means anything**: it asserts `.rail` and `.health-banner` really are translucent and blurred in the same run, so "no reading surface is glass" cannot pass on a page where the glass was never applied. Checked against a deliberate break — glass added to `.transcript` — which fails it with the offending colour named |
| `e2e/fonts.spec.ts` | **the typography, which no test had ever rendered.** `e2e/launch.ts` aborted every request to `fonts.googleapis.com` and `fonts.gstatic.com` in all 22 specs, so both shipped faces were exercised by nothing and every browser assertion in the repository ran on fallback system fonts. Asserts no request reaches either CDN origin, that both `latin` `woff2` files are fetched same-origin, that both `FontFace` entries reach `status: "loaded"` rather than falling back to `system-ui`, and that the two above-the-fold faces are preloaded | **3 passing** against the dev server. The `status: "loaded"` assertion is the one that matters: an unanchored `.gitignore` glob has already cost this repository a source file once, and losing the `woff2` files would otherwise change nothing that fails |
...
| manual real-runtime pass (Slice 6R) | the single-user product on **real MLX Kyutai + OpenAI `gpt-4o-mini`**, with 6R's presentation and corrections in it | **RUN 2026-09-10 on Apple silicon by Aymen.** `/readyz` green with MLX weights loaded, PostgreSQL healthy and OpenAI configured; a real meeting completed through the browser; French transcript and real intelligence generated; the transcript readable as speaker-turn paragraphs with speaker turns preserved; the review page loaded; the correction path exercised. **Not a regression test and not a report** — no artefact, nothing re-runs it, and what it does and does not attest is L-41 |
...
**R0 current limitations:** no live voice implementation, no verified remote GPU
runtime, no cloud deployment, no cross-network meeting. Remote readiness returns
unknown; Docker packaging/audio persistence, guest review, join idempotency,
socket replacement ownership and ASR failure isolation require the scoped work
in the architecture document. These findings do not invalidate named earlier
fake tests, and are not claimed fixed. Original PRD/architecture/engineering
sources remain unavailable; their reconciliation documents and handoff were read.
...
| L-5 | ~~No transcript editing~~ **CLOSED 2026-09-10.** Segments can be corrected — text and speaker — additively | Q9's default was reversed by Aymen on 2026-09-09 and built as Slice 6R item 7. `text` and `words` are never overwritten; the correction lives beside them. What is still *not* editable: timestamps (L-40), and there is no edit history (L-39) | — |
...
| L-9 | Search is not indexed | Prototype scale | Meeting count grows. **Correction 2026-09-10: this row said "Search is ILIKE" and that has been false since rev 16 — see L-33.** Matching is accent-insensitive, in Python, meeting-scoped, and deliberately not `ILIKE`, because `ILIKE` is accent-sensitive and this is a French-first product. The *unindexed* half of this row still stands |
...
| L-34 | The lifecycle chain has an automated walk and a manual one, and they are not the same walk | `test_full_lifecycle.py` walks create → review in one test, but on `MeasuredRecognizer` and `FakeLLMProvider` — ✅ **fake**. Aymen walked the same chain on 2026-09-08, and again on 2026-09-10 with Slice 6R's grouped transcript and corrections in it, against real MLX Kyutai and real `gpt-4o-mini`, and reported it working end to end — ✅ **M1 + MLX**, but by hand, with no artefact beyond that report (L-41). So the chain is *tested* without a model and *demonstrated* with one, and neither half is the other: a passing demo is not evidence (D-03), and a fake's timing is not a model's compute. Accepted because the alternative needs Apple silicon in the test runner, which is L-18 and L-27 rather than anything this test could fix | A CUDA host makes the real path runnable somewhere other than one laptop (Phase B), or someone records a second measured fixture. Until then, read the two halves together and neither alone |
...
| L-37 | `PARAGRAPH_SILENCE_MS` is a guess, and it decides how the transcript reads | 3 500 ms, chosen by reasoning rather than measured: comfortably above the segmenter's own 1 200 ms silence rule, so grouping actually groups, and below what a real pause between topics looks like. **Deliberately not tuned against the fake recognizer**, which is a metronome and would produce a confident wrong number (CLAUDE.md). Marked `[measure]` in the source. The cost of it being wrong is cosmetic — paragraphs that are too long or too short — not a correctness failure, because the underlying segments are untouched | Read the gap distribution off a real MLX run's segments and pick a threshold separating within-thought from between-thought pauses, the way §9.3 was retuned from Spike B1's word timings. Needs Aymen's M1 |
| L-38 | Grouping **hides** L-28's orphaned final words; it does not fix them | The real fixture orphans a sentence's last word into its own 160-400 ms segment, 8 times in 30. Those gaps are far below any paragraph threshold, so a reader stops seeing them — which is the point of the presentation change and also its hazard. The segments are exactly as the model produced them, `test_l28_orphaned_final_word.py` still pins the defect's shape, and `paragraphs.test.ts` asserts the orphan is re-joined *without being altered*. Worth stating loudly because a defect nobody can see is easy to mistake for a defect that was fixed | L-28's own revisit trigger. Grouping changes nothing about when it should be reopened |
| L-39 | A segment keeps the **raw** text and the **latest** correction — not a history of edits | Migration `0002` stores one correction per segment, so editing a segment twice overwrites the first edit. The raw ASR output is preserved, which is the property Q9 actually requires and the one L-28, the 1.43% WER and every `[measure]` row depend on; the intermediate human drafts are not. Chosen because Aymen's item-7 spec said "keep it small" and explicitly ruled out a revision-diff UI for now, and a correction *table* is the shape you would want only if you intend to show that history | Somebody wants to see who changed what and when — most likely alongside Slice 7's real identity model (Q4), since `corrected_by` currently holds a host user id and nothing renders it |
| L-40 | Timestamps are not correctable | `start_ms` / `end_ms` are derived from frame counts (ADR-11) and are what FR-11's audio seeking arithmetic rests on: `offset = segment.start_ms - session.epoch_ms`. A typed number there would desynchronise a citation from its recording — the feature would look like it worked and play the wrong moment. Aymen's spec listed timestamp correction as "maybe later", and this is the reason it should stay later | A real need, and a design that keeps the audio offset derivable — probably shifting the stored epoch rather than the segment |
| L-41 | **The real-runtime correction pass is attested, not recorded** | Aymen ran a full meeting on real MLX Kyutai + OpenAI `gpt-4o-mini` on 2026-09-10 and reported the correction path working on that real French transcript. That is the claim this project most wanted and could not get from a sandbox — and it is a sentence, not an artefact. Slice 4 left `docs/slice4-last-test-report.json`; this left nothing. **So do not read it as narrower than it is written.** It says corrections work on the live path. It does **not** separately attest which behaviours were exercised — whether the original text was checked as preserved in the row afterwards, whether **Régénérer** was pressed against real `gpt-4o-mini` and produced v2 outputs, whether a speaker was reattributed, how many corrections were made. Each of those has a passing ✅ fake test (`test_segment_corrections.py`, `e2e/corrections.spec.ts`); none has a ✅ M1 + MLX one. Accepted because the automated half is thorough and the real half is a person's time; recorded because "validated on M1" will otherwise be cited for things it never covered | Someone re-runs it and writes down what they did — ideally as a report the way Slice 4 did. The regenerate-against-real-provider leg is the one worth capturing first: it is the only correction behaviour that spends money, and A-6's rejection rate and the cost-per-meeting row in §8 are both still unmeasured |
| L-42 | **The contrast script found five shortfalls the first time anything measured this palette. One was a bug and is fixed; four remain** | Recorded 2026-09-14, because a number nobody wrote down is a number nobody fixes. **The bug, now closed: "Changer de jeton" was invisible at rest.** `.btn-quiet` sets `color: var(--ink)` and the rail it sits in is `background: var(--ink)`, so the only authentication affordance in the app was ink on ink — its `--grout` border the only thing showing — until `:hover` painted `--surface` behind it. Found by computation, then **observed in Chromium**: before the fix the button's computed `color` and the rail's computed `background-color` are both `rgb(17, 33, 45)`, exactly 1.00:1. Fixed in F1 on Aymen's call, by two rules that give a quiet button the colour of the surface it sits on: 14.33:1 at rest, 16.42:1 on hover, both now asserted pairs rather than gaps. **The three that remain are visual decisions, not defects**, which is why F1 measured and pinned them instead: the `--grout` hairline that is a text input's only boundary measures 1.39:1 against `--surface` where WCAG 1.4.11 asks 3:1; `.seg-edit` at `opacity: 0.45` measures 2.06:1 at rest, reaching full `--azure` only on hover and focus; and `.evidence-link` at `opacity: 0.8` passes at 8.63:1 but drops to 0.4 when disabled. Each is pinned at today's ratio in `check-contrast.mjs` and fails the build if it gets worse **The rail focus ring is closed too, by F2 and because of F2.** The glaze lightened the rail, which pushed that gap from 2.37:1 to 2.05:1 — a pinned floor breached, which is exactly what the ratchet exists to refuse. The fix is the rule the rest of the rail already follows, `outline-color: var(--porcelain)`, measuring 14.33:1. A gate that only ever said "still fine" would have shipped the regression instead | F2 closed one of these and refused to make another worse; the rest is F3-F5 territory. None of it is load-bearing for the R-series |
...
| Date | Change |
|---|---|
| 2026-09-14 | **rev 32 / F2.** The ground and the glaze. A quiet zellige-derived field behind the app — two low-alpha radial washes and a diamond grout lattice on the 4px module — and `backdrop-filter` glass on chrome only: rail, health banner, status bar, roster, notices, consent. Written fallback-first, so a browser without `backdrop-filter` and a reader with reduced transparency both land on F1's opaque surfaces. The contrast gate was taught the ground's worst case and **caught four regressions the ground introduced**, all fixed rather than waived: the health banner had no background of its own and its technical detail fell to 3.61:1; the rail glaze at 0.94 put `.rail-foot` at 4.44:1; and the rail focus ring breached its pinned floor. 43 pairs, 115 unit tests, typecheck, build and 28 browser specs pass. **Rendered and looked at on four screens — the first time anyone has seen this project's interface as a design.** Reading surfaces stay opaque and dominant; the lattice shows faintly through the glazed chrome. No backend, API or schema change. |
| 2026-09-14 | **rev 31 / F1.** Frontend foundation, a parallel track to R1-R7 and on nobody's critical path. Radii scale completed and all six raw values migrated; status colours and the azure-derived citation washes named, retiring the second blue; the three `prefers-color-scheme: dark` blocks deleted (they were a 1.66:1 health banner on a page with no dark theme); both faces self-hosted and `isolateFromCdns` removed, so the browser specs render the real typography for the first time; `scripts/check-contrast.mjs` added and wired into `npm test`. 30 contrast pairs, 115 frontend unit tests, typecheck, build and 25 browser specs pass; measured layout delta from the real fonts is headline widths only, no reflow. Five pre-existing contrast gaps found and recorded as L-42 — one of them a bug, fixed here on Aymen's call: "Changer de jeton" rendered ink-on-ink in the rail at 1.00:1, confirmed in Chromium before and after. **Nobody has seen any of it in a browser as a design**; review pending. |
...
| 2026-09-10 | **rev 22. Slice 6R items 1–6 built: the transcript reads like minutes.** `ReviewPage` grouped one `<p>` per segment with the speaker repeated on each; an hour of French was ~930 labelled lines. It now groups into speaker-turn paragraphs — speaker and an mm:ss timestamp said **once per turn** — split by a speaker change or a silence over `PARAGRAPH_SILENCE_MS`. **The design constraint did all the work:** a paragraph is *a list of segments, not a joined string*, and the renderer keeps one `<span data-segment-id>` per segment inside it. That is what lets citations still scroll to a moment and search highlights still index into the right segment's text at the server's offsets; joining would have broken both silently and only in a browser. Also: interim text in the live view now carries the word *en cours* rather than relying on italics and a hue, which is a real accessibility fix, not a cosmetic one; and a summary that **succeeded but extracted nothing** is finally its own state instead of rendering a lone paragraph under empty headings, indistinguishable from a page still loading. **The live view is deliberately unchanged otherwise** — word-by-word interim rendering is what makes latency and transcription activity visible, and it is the debugging surface. **The honest part of this revision is that my first two regression tests did not work.** Items 4 and 5 were specified as regressions, and I asserted in the spec's own docstring that they would fail if a paragraph were ever concatenated. Measured against a concatenating rewrite, they **passed**: the citation test clicked the *first* citation, whose target is also its paragraph's first segment and survives collapsing, and the search test picked a word from the *first* segment, whose offsets start at index 0 and therefore line up against a joined string by coincidence. Both were rewritten to use a later segment, and re-measured: the mutation now fails **3 of 6** — grouping, citation, highlight — while the three tests about other behaviour correctly still pass. The first version would have shipped as a guard that guarded nothing, which is the same failure shape as L-35 one layer up. Two limitations recorded rather than smoothed: **L-37**, `PARAGRAPH_SILENCE_MS` = 3 500 ms is a `[measure]` guess deliberately not tuned against the fake; **L-38**, grouping *hides* L-28's orphaned final words and does not fix them — the segments are untouched and the characterization test still pins the defect. **And the full browser run caught a second thing the new specs could not:** `two-participants.spec.ts` broke, because its `transcriptOn` helper read `.speaker` from *inside* each `.line-final`, and grouping moved the name into a sibling paragraph header. It returned an empty speaker rather than erroring, so the failure surfaced as "Amina not found in the transcript". A genuine regression I introduced, fixed in the helper — which now reads its own `.speaker` in the live view and falls back to the paragraph header in the review view — and not by reshaping the markup back, which would have undone the slice. Worth noting that neither the unit tests nor the six new browser specs would have found it; only running the *whole* suite did. Frontend unit 79 -> 91, browser specs 8 -> 14, `tsc` and `npm run build` clean. **The backend is byte-identical to `main` in this revision — `git diff` touches zero files under `backend/`** — and was re-measured anyway: `382 passed, 1 deselected in 100.65s`, with ruff, ruff format and mypy strict (97 files) clean. One detour worth recording so nobody repeats it: an earlier attempt reported `12 failed`, and that was **my own concurrency, not a regression** — I started a second `pytest` against `mosaique_test` while the first was still running, and the two share a database that the `engine` fixture drops and recreates and the `tenants` fixture truncates. Re-run alone, it is green. **Item 7, additive transcript corrections (Q9), is not in this revision** — it is backend work with a migration and is the one 6R item left. |
...
The new order is R0 architecture → R1 deployment specification → R2 real remote
ASR → R3 failure/lifecycle correctness → R4 identity/guest review → R5 WebRTC
voice → R6 deployed integration/metrics → R7 measured two-network acceptance.
Four-user validation and pilot hardening follow separately. No new single-user
presentation work is on this critical path. Existing L-28 deferral and additive
correction rules stand.

### The F-series — frontend track, parallel to R1-R7 (2026-09-14)

`docs/frontend-enhancement-plan.md` carries F1-F5. It is **not** on the R-series
critical path and does not compete with it: the two touch almost disjoint files,
the one overlap being `LiveMeeting.tsx`, which R5 changed and which F5 will.

**F1 and F2 are merged / gated for review.** F1 re-anchored the visual system —
the radii scale, named status colours, citation washes derived from `--azure`
instead of a second blue — deleted the three `prefers-color-scheme: dark` blocks
that were painting a 1.66:1 health banner on a page with no dark theme,
self-hosted both faces so the browser specs stop blocking them, and added
`frontend/scripts/check-contrast.mjs` to `npm test`.

**What F1 deliberately did not do is change how anything looks**, with one
exception Aymen asked for: **"Changer de jeton" rendered ink-on-ink in the rail
at 1.00:1** — invisible, not merely low-contrast — and is fixed. The other four
gaps the script found are recorded (L-42) rather than fixed, because each is a
visual decision and appearance is F2's mandate.

**F2 added the ground and the glaze**, and is the first change in this project
to have been *looked at*: the dashboard, join, live and review screens were
rendered and read at 1280x860. The reading surfaces stay opaque and dominant
and the grout lattice shows faintly through the glazed chrome, which is the
whole thesis — depth by translucency, the way a glaze sits over a coloured
body. Its rule that **glass never touches a text-reading surface** is now held
by `e2e/glass.spec.ts` rather than by intention.

**What is still true: there is no visual regression baseline** in this
repository and no CI to hold one (L-18), so every gate in the plan is run by
hand and four screenshots are not a baseline. The measurements are facts; the
design is still a proposal, and F3-F5 (join, dashboard, live call) are
unstarted. F5 must rebase on R5 and read its diff first — `LiveMeeting.tsx` is
the one file the two tracks share.

### Historical handoff — §15 metrics (2026-09-10; superseded sequencing)

**There is no work in progress.** Slice 6R is complete, validated on the M1, and
merged as PR #16. `claude/sharp-dijkstra-uf34s8` is synced to `main` (`29c7b3d`)
and carries nothing unmerged. Phase A has **exactly one item left and it is
unstarted**: 6A's remaining §15 metrics, item 1 under "The rest of Phase A"
below. Read that item, then start.

After the metrics, Phase B — blocked on a CUDA host, not on code. Do not start
Phase C work (four participants); MLX cannot serve two streams (L-26) and the
sequencing is deliberate.

**Three things to know before you touch anything**, each of which has cost this
project a session already:

1. **Read `CLAUDE.md` "Decisions already made" first.** L-28 is deferred *by
   decision*, Q2's residency half is open and is a product call, and L-33's
   search deviation is deliberate. Re-opening any of them unprompted wastes a
   session.
2. **There is no CI (L-18).** `ruff`, `mypy`, `pytest`, `npm test`, `npm run
   build` and `tools/export_openapi.py` are yours to run before you push, and a
   stale `frontend/openapi.json` means the generated client silently disagrees
   with the server.
3. **Do not run two pytest sessions against `mosaique_test` at once.** The
   `engine` fixture drops tables and `tenants` truncates, so a concurrent run
   produces failures that look like real bugs and are not. This happened during
   rev 23.

### Slice 6R — COMPLETE 2026-09-10, VALIDATED ON M1 THE SAME DAY

Aymen re-ran everything on Apple silicon: migration `0002` clean against his
database, backend `398 passed, 1 deselected`, frontend `91 passed`, typecheck and
build clean, 19 browser specs — and a full manual meeting on real MLX Kyutai +
OpenAI `gpt-4o-mini` in which the transcript read as speaker-turn paragraphs and
**the correction path was exercised on that real French transcript**. That closes
the gap the slice shipped with. **What it does not close is L-41** — read that
before citing "validated on M1" for any specific correction behaviour, because
the pass is an attestation with no artefact behind it.

All seven items built and browser-tested: grouped paragraphs with the speaker and
an mm:ss timestamp said once per turn (`review/paragraphs.ts`), an interim marker
in the live view that does not depend on colour, distinct failed /
extracted-nothing summary states, and **additive segment corrections**. Citations
and search highlighting are held by browser regression tests that were re-written
after measurement showed the first versions did not actually catch a
concatenating rewrite.

**Item 7, and the one thing to understand before touching it.** A correction
never writes `text` or `words` — migration `0002` puts it in `corrected_text` /
`corrected_participant_id` beside them. Every reader goes through
`transcript/corrections.py`, which is the *only* definition of "what this segment
says"; the review page, `?q=` search and the summarizer's prompt all use it. Put
a second copy of that rule anywhere and you get L-35 again, one layer down: a
correction visible on screen and absent from the summary.

Correcting bumps `transcript_version`. Outputs record which version they came
from, and the two differing is what makes the review page say *à revoir* and
offer one button. **Regeneration is never automatic** — that was Aymen's call and
`test_correcting_does_not_re_run_the_summary_by_itself` asserts it.

Not built, deliberately: correction *history* (L-39) and timestamp correction
(L-40). Both have reasons recorded in §10; neither is an oversight.

### Slice 6R in one paragraph — *why it existed*, kept as history

Written before the slice; left here because the principle outlived it. `ReviewPage`
**used to render** one `<p>` per segment with the speaker's name repeated on every
one. Segments close on 1 200 ms of silence, so an hour of French was ~930 labelled
lines and 8 in 30 a single orphaned word (L-28) — a log, not a transcript. The fix
was presentational, not structural: **preserve the raw segment and word evidence
internally, present a human-readable transcript by default.** ADR-05 did not
change. The live view keeps its word-by-word interim rendering on purpose — that
is the debugging and auditing surface. Two of the seven items (clickable
citations, search highlighting) already worked and were *regression constraints*,
because grouping is exactly what breaks them.

**That principle now governs anything that touches the transcript.** Present the
readable thing, keep the raw thing, and never let the presentation layer become
the record.
...
**1. The remaining §15 metrics, with a stated reason each. ← THE ONLY OPEN ITEM.**
...
*Starting points, so this does not have to be re-derived:* read tech spec §15 for
the list itself, and `app/api/health.py` plus `tests/unit/test_health_probes.py`
for the shape the existing half settled on — injected probes, tested without a
server. The seams constrain where the emitting code can live: nothing downstream
of the ingress may import FastAPI or Starlette, and `test_architecture.py` will
fail the build if a counter is incremented from the wrong layer. Expect that to
be the design problem here, since the interesting numbers (frames received,
first-word latency) are produced deep in the runtime and exposed at the transport.

*Two of these metrics already have measured values in §8 and one does not:*
first-word latency p95 is **1 934 ms** on real MLX, final-segment latency has
never been measured at all, and the difference matters — the 2026-09-08 run
reports *pipeline* latency, which is a different quantity. Do not wire a metric
name to the quantity it sounds like without checking §8's row for it.
