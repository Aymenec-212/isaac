...
**Last updated:** 2026-09-10 (rev 22 — Slice 6R items 1–6 built)
**Updated by:** Slice 6R session
**Current slice:** **Slice 6R — single-user review hardening. PHASE A.**
Specified 2026-09-09, **six of its seven items built 2026-09-10**. Item 7,
transcript corrections, is the one left — see `docs/IMPLEMENTATION_PLAN.md` v1.4. Slice 6A is five of six done (health
...
| Next action | **Slice 6R item 7 — additive transcript corrections (Q9).** Items 1–6 are built and browser-tested: grouped paragraphs with speaker and timestamp said once per turn, an interim marker that does not rely on colour, and distinct failed/thin summary states — with citations and search highlighting held as regressions that now genuinely fail if a paragraph is concatenated. Item 7 is backend work: a correction stored *alongside* the raw ASR text, never overwriting it. **Then 6A's last item, the §15 metrics.** Needs Aymen's M1 and unchanged: real-time factor, first-word and final-segment latency, memory growth, long-run MLX stability. **Start from §12** |
...
### Slice 6R — items 1–6 done 2026-09-10, item 7 left

Built and browser-tested: grouped paragraphs with the speaker and an mm:ss
timestamp said once per turn (`review/paragraphs.ts`), an interim marker in the
live view that does not depend on colour, and distinct failed / extracted-nothing
summary states. Citations and search highlighting are held by browser regression
tests that were **re-written after measurement showed the first versions did not
actually catch a concatenating rewrite** — they now do.

**Item 7 is what remains: additive transcript corrections (Q9).** Backend work —
a correction stored alongside the raw ASR text, never overwriting it, with
authorship and a timestamp. The open question inside it, deliberately not
pre-decided: whether correcting a cited segment should invalidate or re-run the
meeting intelligence quoting it.
