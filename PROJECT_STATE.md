...
| Tests passing | **405**: 276 backend unit, 67 backend integration and realtime (real PostgreSQL), 60 frontend unit, 2 Playwright browser specs. Plus one opt-in accelerated hour behind `-m slow`. Backend re-run in this session — `343 passed, 1 deselected in 93.85s`; frontend `60 passed`; `npm run build` clean. The 2 Playwright specs are carried forward from rev 9 and were **not** re-run against the new review page (L-32). **None of these executes a model or a provider**: the ASR evidence is a report (L-27) and the LLM has never been called (L-31) |
...
| Next action | **Slice 6A, continued: transcript search (FR-10)** — the last functional requirement a single participant can exercise. Health endpoints are done (see below); after search comes a long single-user run against the real MLX runtime, then the full-lifecycle test |
...
| FR-10 | Transcript review and search | read: Slice 1, search: **Slice 6A** | **read VERIFIED; search NOT BUILT** | `GET /transcript` covered, ordered across participants by `start_ms`. Search is the last functional requirement a single participant can exercise and is Phase A work |
...
| `observability/health.py` + `/readyz`, `/health/deps` | §15 | **VERIFIED** ✅ fake | `test_health_probes.py` (11), `test_health_endpoints.py` (10). Per-dependency; the LLM provider is listed and does not gate readiness, per §15 |
...
| `frontend src/health/` banner | §15 | **VERIFIED** ✅ fake (unit) / ⚠ never seen in a browser | `status.test.ts` (11). The component itself has no browser test — L-32 |
...
4. **A full-lifecycle test** — create, join, speak, end, finalize, summarize,
   review — run end to end rather than as pieces that each pass separately.
